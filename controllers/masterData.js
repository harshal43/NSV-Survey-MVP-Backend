...
const projectDeatils = async (req, res) => {
  const client = await pool.connect();
  const currentTimestamp = Date.now();
  const content = req.body;
  try {
    ...
    const getList = await client.query(getProjectDeatils, [project_id]);
...
export { roList, PiuList, ProjectList,laneData ,projectDeatils};
